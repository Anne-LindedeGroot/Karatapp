...
class OhyoCardInteractions extends StatefulWidget {
  final Ohyo ohyo;

  const OhyoCardInteractions({
    super.key,
    required this.ohyo,
  });

  @override
  State<OhyoCardInteractions> createState() => _OhyoCardInteractionsState();
}

class _OhyoCardInteractionsState extends State<OhyoCardInteractions> {
            ...
            OverflowSafeRow(
              children: [
                ...
                IconButton(
                  onPressed: () async {
                    try {
                      await ref.read(ohyoInteractionProvider(widget.ohyo.id).notifier).toggleFavorite();
                      if (mounted && context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
  ...
  Widget _buildClickableCommentPreview(OhyoComment comment) {
